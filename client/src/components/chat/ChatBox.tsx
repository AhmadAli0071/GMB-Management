  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl overflow-hidden border border-slate-200">
      {!selectedUserId ? (
        <>
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 shrink-0">
            <div className="flex items-center justify-between">
              ...
              <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${connected ? 'bg-blue-500' : 'bg-red-500'}`} />
            </div>
          </div>
               ...
               otherUsers.map((user: any, idx: number) => {
                 ...
                 return (
                   <button
                     ...
                     className="w-full flex items-center gap-3 px-4 py-3.5 bg-blue-500 rounded-2xl transition-colors hover:bg-blue-600"
                   >
                     <div className={`w-11 h-11 rounded-full bg-gradient-to-br ${avatarGradient} flex items-center justify-center text-base font-bold text-white shrink-0 shadow-md`}>
                       {user.name.charAt(0).toUpperCase()}
                     </div>
                     <div className="flex-1 min-w-0 text-left">
                       <div className="flex items-center justify-between">
                         <span className="truncate text-[15px] text-white font-semibold">{user.name}</span>
                         {unread > 0 && (
                           <span className="bg-white text-blue-600 text-[11px] font-bold rounded-full min-w-[20px] h-5 flex items-center justify-center px-1.5 shrink-0">
                             {unread > 99 ? '99+' : unread}
                           </span>
                         )}
                       </div>
                       <p className="text-[12px] text-white/70 truncate mt-0.5">{ROLE_LABELS[user.role as UserRole] || user.role}</p>
                     </div>
                   </button>
                 );
               })
        ...
        <>
          <div className="px-3 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center gap-3 shrink-0">
            <button
              onClick={() => { setSelectedUserId(''); setMessages([]); setEditingId(null); setMenuOpenId(null); setShowClearConfirm(false); }}
