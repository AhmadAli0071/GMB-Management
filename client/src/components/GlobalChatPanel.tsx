...
import { Send, Paperclip, Mic, Download, MoreVertical, Edit3, Trash2, EyeOff, Square, Search, ChevronLeft } from 'lucide-react';
...
export function GlobalChatPanel({ onClose }: GlobalChatPanelProps) {
  ...
  const { joinDMGlobal, leaveDMGlobal, onDMMessage, offDMMessage, onDMMessageEdited, offDMMessageEdited, onDMMessageDeleted, offDMMessageDeleted, onDMChatCleared, offDMChatCleared, onDMNotification, offDMNotification } = useSocket();
  ...
  return (
    <div className="flex h-[70vh] bg-white rounded-xl overflow-hidden border border-slate-200">
      {/* Users List Sidebar */}
        ...
        {selectedUser ? (
          <>
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center gap-3 shrink-0">
              {onClose && (
                <button onClick={onClose} className="md:hidden p-1.5 hover:bg-slate-100 rounded-lg text-slate-500">
                  <ChevronLeft size={20} />
                </button>
              )}
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[12px] text-slate-500">{ROLE_LABELS[selectedUser.role as UserRole] || selectedUser.role}</span>
              </div>
